...
def clustered_comp():
    ...
    for source in sources:
        ...
        for iter in range(0, len(score_df.columns)):
            ...
            plt.savefig('./results/Syn/plots/clustered_syns/' + source + '_synergy_' + str(iter) + '.svg',
...
def get_best_params_hier(type, discard):
    """
    THIS FUNCTION IS DEPRECATED
    It was used to get hyperparameters that obtained the best classification accuracy
    when discarding synergies and using a hierarchical paradigm
    """
...
def get_best_params_multi(type, discard):
    ...
    THIS FUNCTION IS DEPRECATED
    It was used to get hyperparameters that obtained the best classification accuracy
    when discarding synergies and using multiple sources
    """
...
def get_best_params_single(type, discard):
    ...
    THIS FUNCTION IS DEPRECATED
    It was used to get hyperparameters that obtained the best classification accuracy
    when discarding synergies and using single source
    """
...
def score_reordering(type):
    """
    This function reorders the scores based on the previously generated clusters
    THIS FUNCTION IS DEPRECATED
    """
...
def syn_clustering():
    ...
    It is based on Agglomerative Clustering
    THIS FUNCTION IS DEPRECATED
    """
...
def syn_clustering_alternative():
    ...
    This function is created to pair synergies among subjects
    It uses a recursive approach
    It is based on correlation between synergies
    THIS FUNCTION IS DEPRECATED
    """
    # sources = ['kin', 'emg_pca', 'tact']
    sources = ['kin']
...
def kin_syn_extraction(data):
    """
    This function extracts the synergies from kinematic data and saves it
    in the way it is used by Simon (or our internal format if needed)
    """
...
def syn_extraction(data):
    ...
    Now we only extract syns for kinematic data
    """
...
def kin_syn_classif(input_data):
    """
    THIS FUNCTION IS DEPRECATED
    It was used ato build the classifiers based on single source synergies
    progressively discarding them.
    The classifier bins every trial and targets the 'Given' object
    """
...
def syn_single_source_classification(type, discard):
    """
    THIS FUNCTION IS DEPRECATED
    It was used as interface call the classifiers based on single source synergies
    progressively discarding them.
    The classifier bins every trial and targets the 'Given' object
    """
...
def hierarchical_syn_classification(type, discard):
    """
    THIS FUNCTION IS DEPRECATED
    It was used as interface call the classifiers based on synergies from each type of signal
    (kinematic + emg + tactile) based on a hierarchical paradigm progressively discarding the synergies.
...
def multi_aux_classification(input_data):
    """
    THIS FUNCTION IS DEPRECATED
    It was used to build the classifiers based on multisource synergies (Kin + EMG + tactile)
    progressively discarding them.
    The classifier bins every trial and targets the 'Given' object
    """
...
def multisource_syn_classification(type, discard):
    """
    THIS FUNCTION IS DEPRECATED
    It was used as interface call the classifiers based on multisource synergies (Kin + EMG + tactile)
    progressively discarding them.
    The classifier bins every trial and targets the 'Given' object
    """
    perc_syns = [100, 90, 80, 70, 60, 50, 40, 30, 20, 10]
...
def print_syn_results(type, discard):
    """
    THIS FUNCTION IS DEPRECATED
    It was used to generate the plots associated to the classifiers when discarding synergies
    """
    ...
    combined_cum_var = [statistics.mean(k) for k in zip(kin_cum_var)]
    ...
    if discard == 'least':
        title += '\ndiscarding the least relevant synergies'
        fig_file += '.png'
    else:  # discard the most
        title += '\ndiscarding the most relevant synergies'
        fig_file += '_decr.png'
    ...
    sns.pointplot(data=pd.DataFrame(extended_kin_cum_var).transpose(), label='Variance Explained', color='r', scale=.5, markers='*')
...
def early_fine_vs_coarse():
    ...
    plt.savefig('./results/Early Enclosure/plots/fine_vs_coarse.svg', format='svg', dpi=600)


def syn_fine_vs_coarse_fam(type):
    """
    These functions compare the synergy scores for low and high order synergies between fine and coarse families
    ...
    # tit = 'Low and High order recruitment for synergies from'
    # if type == 'cluster':
    ...
    if type == 'cluster':
        ...
        plt.savefig('./results/Syn/plots/clust_fine_vs_coarse_fam.svg', format='svg', dpi=600)
    else:
        # plt.savefig('./results/Syn/plots/all_fine_vs_coarse_fam.png', dpi=600)
        plt.savefig('./results/Syn/plots/all_fine_vs_coarse_fam.svg', format='svg', dpi=600)


def syn_fine_vs_coarse_ep(type):
    """
    These functions compare the synergy scores for low and high order synergies between fine and coarse EPs
    'Cluster' is deprecated
    """
    ...
    fine_eps = ['enclosure part', 'rotation', 'function test', 'contour following', 'translation']
    ...
    if type == 'cluster':
        ...
        plt.savefig('./results/Syn/plots/clust_fine_vs_coarse_ep.svg', format='svg', dpi=600)
    else:
        # plt.savefig('./results/Syn/plots/all_fine_vs_coarse_ep.png', dpi=600)
        plt.savefig('./results/Syn/plots/all_fine_vs_coarse_ep.svg', format='svg', dpi=600)

def print_syn_results_alternative(discard):
    """
    This function is used to create the plots associated to classifiers using raw data
    and synergies when discarding them
    """
    ...
    # Ensure the variance data has the correct number of points
    if extended_kin_cum_var.shape[1] < len(perc) + 1:
        extended_kin_cum_var = extended_kin_cum_var.reindex(columns=range(len(perc) + 1),
                                                            fill_value=extended_kin_cum_var.iloc[0, -1])
    ...
    # sns.pointplot(data=pd.DataFrame(extended_kin_cum_var).transpose(), label='Variance Explained', color='r', scale=.5, markers='*')
    i = sns.barplot(data=kin_pval_df)
    sns.pointplot(data=extended_kin_cum_var, label='Variance Explained', color='r', scale=.5, markers='o')
    # pairs_kin = [('Raw', '100'), ('Raw', '90'), ('Raw', '80'), ('Raw', '70'), ('Raw', '60'), ('Raw', '50'), ('Raw', '40'), ('Raw', '30'), ('Raw', '20'), ('Raw', '10')]
    pairs_kin = [('Raw', 100), ('Raw', 90), ('Raw', 80), ('Raw', 70), ('Raw', 60), ('Raw', 50),
                 ('Raw', 40), ('Raw', 30), ('Raw', 20), ('Raw', 10)]
