...
def load(subject):
    """
    This function loads the data form a single subject.
    As it is, it only loads data from CyberGlove system
    and the EP labels.
    """
...
def load_eps(subject):
    """
    This function generates, for each trial corresponding to a subject, 3 different dataframes.
    Each dataframe is encoding for each trial executed, information about the different EPs in taht trial.
    This information is, depending on the dataset, the presence or absence of the different EPs (as binary vector),
    the amount of time spent on each EP or the amount of times each EP is executed in that trial.
    It also includes the asked and given abject as well as the family.
    """
